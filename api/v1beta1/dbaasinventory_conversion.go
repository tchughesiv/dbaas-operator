...
// ConvertTo converts this DBaaSInventory to the Hub version (v1alpha1).
func (src *DBaaSInventory) ConvertTo(dstRaw conversion.Hub) error {
	dst := dstRaw.(*v1alpha1.DBaaSInventory)

	// ObjectMeta
	dst.ObjectMeta = src.ObjectMeta

	// Spec
	dst.Spec.CredentialsRef = (*v1alpha1.LocalObjectReference)(src.Spec.CredentialsRef)
	...
	dst.Spec.ProviderRef = v1alpha1.NamespacedName(src.Spec.ProviderRef)

	// Status
	dst.Status.Conditions = src.Status.Conditions
	for i := range src.Status.Instances {
		dst.Status.Instances = append(dst.Status.Instances, v1alpha1.Instance(src.Status.Instances[i]))
	}
...
func (dst *DBaaSInventory) ConvertFrom(srcRaw conversion.Hub) error {
	src := srcRaw.(*v1alpha1.DBaaSInventory)
...
func setPolicyObj(dst *DBaaSInventory) {
	if dst.Spec.Policy == nil {
		dst.Spec.Policy = &DBaaSInventoryPolicy{}
	}
}
